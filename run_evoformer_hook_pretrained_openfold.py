...
from openfold.utils.evoformer_instrumentation import (
    InstrumentationConfig,
    EvoformerRecorder,
    attach_evoformer_block_output_hooks,
    remove_hooks,
)
...
def main(args):
    ...
    instrument_layers = None
    if args.instrument_layers:
        instrument_layers = [int(x.strip()) for x in args.instrument_layers.split(",") if x.strip()]

    instrument_out_dir = args.instrument_out_dir
    if args.instrument_evoformer and not instrument_out_dir:
        instrument_out_dir = os.path.join(args.output_dir, "instrumentation")
    ...
    for model, output_directory in model_generator:
        evoformer_recorder = None
        evoformer_hook_handles = []

        if args.instrument_evoformer:
            evoformer_recorder = EvoformerRecorder(
                InstrumentationConfig(
                    layers=instrument_layers,
                    capture_msa=True,
                    capture_pair=True,
                    clone=True,
                    to_cpu=True,
            ...
            evoformer_hook_handles = attach_evoformer_block_output_hooks(model, evoformer_recorder)
            logger.info(
                f"Evoformer instrumentation enabled. Layers: "
                f"{instrument_layers if instrument_layers is not None else 'all'}"
            )
        ...
        for (tag, tags), seqs in sorted_targets:
            ...
            if args.instrument_evoformer and evoformer_recorder is not None:
                os.makedirs(instrument_out_dir, exist_ok=True)
                instrument_path = os.path.join(
                    instrument_out_dir,
                    f"{output_name}_evoformer_reps.pt"
                )
                evoformer_recorder.save(instrument_path)
                logger.info(f"Evoformer representations saved to {instrument_path}")
                evoformer_recorder.clear()
        ...
        if args.instrument_evoformer and evoformer_hook_handles:
            remove_hooks(evoformer_hook_handles)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--triangle_residue_idx", default=None, type=int
    )
    parser.add_argument(
        "--instrument_evoformer", action="store_true", default=False,
    )
    parser.add_argument(
        "--instrument_out_dir", type=str, default="",
    )
    parser.add_argument(
        "--instrument_layers", type=str, default="",
    )
    parser.add_argument(
        "--enable_chunking", action="store_true", help="Enable chunking (by default, chunk_size is set to None)"
    )
